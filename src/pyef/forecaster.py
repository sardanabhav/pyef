...
class Forecaster:
    ...
    def get_forecast(self) -> None:
        ...
        col_diff = list(
            set(list(self.X_train.columns)) - set(list(self.X_test.columns))
        )
        logger.debug("fitting model")
        logger.debug(f"different cols = {col_diff}")
        try:
        ...
        if self.trained:
            ...
            try:
                self.pred = pd.DataFrame(
                    self.model.predict(X=self.X_test), index=self.X_test.index
                )
            ...
            try:
                self.pred_in_sample = pd.DataFrame(
                    self.model.predict(X=self.X_train), index=self.X_train.index
                )
                self.pred_in_sample.columns = ["forecast"]
                self.pred_in_sample["actuals"] = self.y_train
                if self.log_target:
                    self.pred_in_sample["forecast"] = np.exp(
                        self.pred_in_sample["forecast"]
                    )
            except Exception as e:
                logger.error(e)
            ...
            logger.error("model not trained. Please call <obj>.train first")
            ...
            if self.trained:
                ...
                try:
                    pred_group = pd.DataFrame(
                        self.model.predict(X=test_group), index=test_group.index
                    )
                ...
                try:
                    pred_group_in_sample = pd.DataFrame(
                        self.model.predict(X=group), index=group.index
                    )
                    pred_group_in_sample.columns = ["forecast"]
                    # pred_group_in_sample['actuals'] = self.y_train.groupby().values
                    if self.log_target:
                        pred_group_in_sample["forecast"] = np.exp(
                            pred_group_in_sample["forecast"]
                        )
                    self.pred_in_sample = pd.concat(
                        [self.pred_in_sample, pred_group_in_sample]
                    )
    ...
    def _data_prep(self) -> None:
        ...
        future_usage = self.feature_dataset.loc[
            self.feature_dataset.index >= self.pred_start, self.data.target_col
        ]
        if future_usage.isna().all():
            self.feature_dataset.loc[
                self.feature_dataset.index > self.pred_start, self.data.target_col
            ] = 0.001
        else:
            self.feature_dataset.loc[
                self.feature_dataset[self.data.target_col].isna(), self.data.target_col
            ] = 0.01
        ...
        if self.pca and self.pca_components:
            logger.warn(
                f"running PCA. Features will be reduced to {self.pca_components}"
            )
    ...
    def _split_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        df_test = df_split.loc[
            (df_split.index >= self.pred_start) & (df_split.index < self.pred_end), :
        ]
