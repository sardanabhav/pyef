...
class Forecaster:
    ...
    def get_forecast(self) -> None:
        ...
        col_diff = list(
            set(self.X_train.columns) - set(self.X_test.columns),
        )
        ...
        if self.trained:
            ...
            try:
                self.pred_in_sample = pd.DataFrame(
                    self.model.predict(X=self.X_train),
            ...
            if self.trained:
                ...
                try:
                    pred_group = pd.DataFrame(
                        self.model.predict(X=test_group),
    ...
    def _data_prep(self) -> None:
        ...
        future_usage = self.feature_dataset.loc[
            self.feature_dataset.index >= self.pred_start,
        ...
        if future_usage.isna().all():
            ...
            self.feature_dataset.loc[
                self.feature_dataset[self.data.target_col].isna(),
                self.data.target_col,
            ] = 0.01
    ...
    def _split_data(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        df_train = df_split.loc[
            (df_split.index >= self.pred_start - self.max_history) & (df_split.index < self.pred_start),
            :,
        ]
    ...
    def plot(self) -> dict[str, Any]:
        if self.predicted:
            ...
            logger.warning("Prediction not made. call get_forecast")
