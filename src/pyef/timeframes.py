...
class EnergyTimeFrame:
    ...
    def _validate(
        ...
        if self._validate_series(self._original_series['kwh_series'], 'kwh') & self._validate_series(self._original_series['weather_series'], 'weather'):
    ...
    def _clean_data(
        ...
        self._kwh_series.sort_index().interpolate(
            method=get_option(f'preprocessing.kwh.fill_na'),
            limit_direction='forward',
            axis=0
        ) 
        self._weather_series.sort_index().interpolate(
            method=get_option(f'preprocessing.weather.fill_na'),
            limit_direction='forward',
            axis=0
        )

    def _create_feature_dataset(
        self
    ) -> None:
    ...
    def _add_mas(
        ...
        for col, mas in mas_dict.items():
            if isinstance(mas, int):
                new_cols[f'{col}_ma_{mas}'] = df.loc[:, f'{col}'].rolling(24 * mas * int(60/self._get_freq(df))).mean()
            else:
                for ma in mas:
                    new_cols[f'{col}_ma_{ma}'] = df.loc[:, f'{col}'].rolling(24 * ma * int(60/self._get_freq(df))).mean()
    ...
    def _add_dd(
        ...
        if method == 'niave':
            daily_max_temp = pd.DataFrame(
                self._weather_series.groupby(
                    self._weather_series.index.date
                )[f'{self.temperature_col}'].max()
            )
            daily_min_temp = pd.DataFrame(
                self._weather_series.groupby(
                    self._weather_series.index.date
                )[f'{self.temperature_col}'].min())
            ...
            daily_avg_temp = pd.DataFrame(self._weather_series.groupby(self._weather_series.index.date)[f'{self.temperature_col}'].mean()).reset_index()
            daily_avg_temp.columns = ['datetime', 'avg_temperature']
            df_merged = self._weather_series.merge(
                daily_avg_temp['avg_temperature'],
                left_on=self._weather_series.index.date,
        ...
        df_merged.loc[
            ...
            'hdd'
        ] = hdd_ref_remp - df_merged.loc[
                hdd_ref_remp - df_merged['avg_temperature'] > 0,
                'avg_temperature'
            ]
        df_merged.loc[hdd_ref_remp -
                      df_merged['avg_temperature'] < 0, 'hdd'] = 0
        
        df_merged.loc[df_merged['avg_temperature'] - cdd_ref_remp >= 0, 'cdd'] = cdd_ref_remp - \
            df_merged.loc[df_merged['avg_temperature'] -
                          cdd_ref_remp > 0, 'avg_temperature']
        df_merged.loc[df_merged['avg_temperature'] -
                      cdd_ref_remp < 0, 'cdd'] = 0
