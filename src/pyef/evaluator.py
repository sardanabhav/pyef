...
class Evaluator():
    def __init__(
        ...
        if not self.forecaster.predicted:
            logger.error(
                f"model not trained. Please call forecaster <obj>.train before calling evaluator class")
...
class ModelGridSearch():
    ...
    def _new_search(
        ...
        for model in self.models:
            @memfile(filepath=f'{self.file_path}/result_{model.__str__().replace("(", "").replace(")", "")}.jsonl', skip=True)
            def _execute_model(
                M: str,
                pred_start: datetime,
                horizon: int
                ...
                if self.save_plots:
                    ...
                    prediction.to_csv(f'{self.file_path}/predictions{model.__str__().replace("(", "").replace(")", "")}.csv', mode='a')
                metrics = Evaluator(forecaster=forecast)
                return {
    ...
    def get_best_model(
        ...
        self.run_search()
        avg_out_of_sample = self.grid_out_of_sample.groupby(
            ['M', 'model']).mean()
