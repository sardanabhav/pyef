...
def check_api(ctx: Context) -> None:
    ...
    ctx.run(
        griffe_check("pyef", search_paths=["src"]),  # color=True),
        title="Checking for API breaking changes",
        command="griffe check -ssrc pyef",
        nofail=True,
    )
...
@duty
def test_not_slow(ctx: Context, match: str = "") -> None:
    """Run the test suite.
    ...
    ctx.run(
        ...
        title=pyprefix("Running quick tests (Not Slow)"),
        command=f"pytest -c config/pytest.ini -n auto -m not slow -k{match!r} --color=yes tests",
    )
